package com.lightbend.lagom.scaladsl.sqs.broker
...
import com.lightbend.lagom.scaladsl.broker.sqs.LagomSqsComponents
import com.lightbend.lagom.scaladsl.client.ConfigurationServiceLocatorComponents
import com.lightbend.lagom.scaladsl.persistence.AggregateEvent
import com.lightbend.lagom.scaladsl.server._
import com.lightbend.lagom.scaladsl.sqs.broker.ScaladslSqsApiSpec._
...
class ScaladslSqsApiSpec
  ...
  private val application = {
    new LagomApplication(LagomApplicationContext.Test) with AhcWSComponents with LagomSqsComponents
    with ConfigurationServiceLocatorComponents {
      override lazy val offsetStore: OffsetStore = new InMemoryOffsetStore
  ...
  private var sqsServer: Option[SQSRestServer] = None

  override def beforeAll(): Unit = {
    super.beforeAll()

    val server = SQSRestServerBuilder.withActorSystem(application.actorSystem).start()
    sqsServer = Some(server)
